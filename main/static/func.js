(powerAVGchartData = 0), (hourEnergyData = []), (dailyEnergyData = []);
var refreshGraphs = 0,
    cnt = 60;
...
function update_ints_count() {
    $.ajax({ url: "/updateData" }).done(function (e) {
        ...
        var r = ((e.I1 > 32767 ? e.I1 - 65535 : e.I1) / 100).toFixed(2),
            o = ((e.I2 > 32767 ? e.I2 - 65535 : e.I2) / 100).toFixed(2),
            d = ((e.I3 > 32767 ? e.I3 - 65535 : e.I3) / 100).toFixed(2);
        (document.getElementById("I1").textContent = r), (document.getElementById("I2").textContent = o), (document.getElementById("I3").textContent = d);
        var i = ((e.P1 > 32767 ? e.P1 - 65535 : e.P1) / 1e3).toFixed(2),
            l = ((e.P2 > 32767 ? e.P2 - 65535 : e.P2) / 1e3).toFixed(2),
            s = ((e.P3 > 32767 ? e.P3 - 65535 : e.P3) / 1e3).toFixed(2);
        (document.getElementById("P1").textContent = i), (document.getElementById("P2").textContent = l), (document.getElementById("P3").textContent = s);
        var y = ((e.S1 > 32767 ? e.S1 - 65535 : e.S1) / 1e3).toFixed(2),
            u = ((e.S2 > 32767 ? e.S2 - 65535 : e.S2) / 1e3).toFixed(2),
            g = ((e.S3 > 32767 ? e.S3 - 65535 : e.S3) / 1e3).toFixed(2);
        (document.getElementById("S1").textContent = y),
            (document.getElementById("S2").textContent = u),
            (document.getElementById("S3").textContent = g),
            e.HDO > 0
            ...
            (document.getElementById("ACTUAL_CONFIG_CURRENT").textContent = e.ACTUAL_CONFIG_CURRENT),
            (document.getElementById("ACTUAL_OUTPUT_CURRENT").textContent = e.ACTUAL_OUTPUT_CURRENT),
            (document.getElementById("EV_STATE").textContent = e.EV_STATE),
            cnt >= 60 ? (refreshEnergyChartHourly(), (cnt = 0)) : cnt++,
            "green" == document.getElementById("refresh").style.backgroundColor ? (document.getElementById("refresh").style.backgroundColor = "") : (document.getElementById("refresh").style.backgroundColor = "green"),
...
function getTime() {
    var d = new Date(),
        ...
        hour = d.getHours(),
        minute = d.getMinutes(),
        secunde = d.getSeconds();

    return [day,month,year, hour,minute,secunde];
}
...
function refreshEnergyChartHourly() {
    ...
    for (var e = 0, t = 0, n = 0, a = 0, r = 0; r < 24; r++)
        null != hourEnergyData[3 * r + 3]
            ? ((e = hourEnergyData[3 * r + 1]),
              (dataP = hourEnergyData[3 * r + 2]),
              (dataN = hourEnergyData[3 * r + 3]),
              console.log("EnergyData: ", hourEnergyData, "Hour: ", e, "dataP: ", dataP),
              (n = n + dataP - dataN),
              (a += 1),
              (energyGraphHourly.data.labels[24 - ((len - 1) / 3 - r)] = e + 1 < 10 ? "0" + e + "-0" + (e + 1) : e + 1 == 10 ? "09-10" : e + "-" + (e + 1)),
              (energyGraphHourly.data.datasets[0].data[24 - ((len - 1) / 3 - r)] = dataP),
              (energyGraphHourly.data.datasets[1].data[24 - ((len - 1) / 3 - r)] = -dataN))
            : (e < 23
                  ? ((e += 1),
              ...
              t > 23 && (t = 0));
    for (r = 0; r < 24; r++) energyGraphHourly.data.datasets[2].data[23 - r] = (n / a).toFixed(1);
...
function refreshEnergyChartDaily() {
    ...
    days = Last31Days();
    for (var r = 0; r < 31; r++)
        null != dailyEnergyData && null != dailyEnergyData[t - r]
            ? ((arr = dailyEnergyData[t - r].split(":")),
              (e = arr[0]),
              ...
              (a += 1),
              (energyGraphDaily.data.datasets[0].data[30 - r] = parseFloat(dataP / 100).toFixed(1)),
              (energyGraphDaily.data.datasets[1].data[30 - r] = -parseFloat(dataN / 100).toFixed(1)))
            : ((energyGraphDaily.data.labels[30 - r] = days[r]), (energyGraphDaily.data.datasets[0].data[30 - r] = 0), (energyGraphDaily.data.datasets[1].data[30 - r] = 0));
    for (r = 0; r < 31; r++) {
        var o = (n / a).toFixed(1);
        energyGraphDaily.data.datasets[2].data[30 - r] = o;
    }
...
function formatDate(e) {
    var t = e.getDate(),
        n = e.getMonth() + 1,
        a = e.getFullYear().toString().substr(-2);
    return t < 10 && (t = "0" + t), n < 10 && (n = "0" + n), (e = (n + "/" + t + "/" + a).toString());
}
$(function () {
    $("div.mainContainer").load("datatable", function () {
        ...
        let i = document.getElementById("energyGraph_daily"),
            l = t.getConfig(31);
        (energyGraphDaily = new Chart(i, l)),
            update_ints_count(),
            setTimeout(function () {
                loadPowerChart();
            }, 2e3),
            ...
            "main" == $(this).attr("id")
                ? (stop(timer),
                  $("div.mainContainer").load("datatable", function () {
                      ...
                      let a = new energyChart("Hourly E [Wh]", "Wh"),
                          r = document.getElementById("energyGraph_hourly"),
                          o = a.getConfig(24);
                      energyGraphHourly = new Chart(r, o);
                      let d = new energyChart("Daily  E [kWh]", "kWh"),
                          i = document.getElementById("energyGraph_daily"),
                          l = d.getConfig(31);
                      (energyGraphDaily = new Chart(i, l)),
                          update_ints_count(),
                          setTimeout(function () {
            ...
            e
                ? $.ajax({
                      ...
                      success: function (t) {
                          ...
                          console.log("Syncing proccess: ",(t.process))
                          $("#synchroTime").find("span").remove();
                      },
                  })
                      ...
                      success: function (t) {
                          $("#updateWificlient").html(t.datalayer)
                                if(t.process == 0){
                                    ...
                                    document.getElementById("wifiStatus").innerHTML = "Can not connect to Wattmeter SSID"
                                    document.getElementById("wifiStatus").style.color = "#FF0000"
                                }else if(t.process == 2){
                                    document.getElementById("wifiStatus").innerHTML = "Currently connected to: "+ e
                                    document.getElementById("wifiStatus").style.color = "#74DF00"
                                }else if(t.process == 3){
                                    document.getElementById("wifiStatus").innerHTML = "Currently connected to: "+ e
                                    document.getElementById("wifiStatus").style.color = "#74DF00"
                                }else{ 
                                    document.getElementById("wifiStatus").innerHTML = "Error during connection to: "+ e
                                    document.getElementById("wifiStatus").style.color = "#FF0000"
                                }
                      },
                  })
